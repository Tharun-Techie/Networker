    ...
    <ul className="list">
      {rows.map((r) => (
        <li key={r.id}>
          ...
          <span className="grow mono">{r.from_id} → {r.to_id}</span>
          <span className={`badge ${CONF_CLASS[r.confidence] ?? "b-unconfirmed"}`}>{r.confidence}</span>
        </li>
      ))}
    </ul>
  );
...
/** Organization profile: ownership/management/board lens over the 1-hop graph. */
export default function OrgProfile() {
  ...
  const people = graph.edges.filter(
    (e) => !board.includes(e) && !ownership.includes(e));

  return (
    <div>
      <div className="card">
        <div className="profile-head">
          <div className="avatar org">{(me?.name ?? id).slice(0, 2).toUpperCase()}</div>
          <div>
            <h2>{me?.name ?? id}</h2>
            <p className="subtle" style={{ margin: 0 }}>
              {[me?.attributes?.industry, me?.attributes?.location].filter(Boolean).join(" · ")
                || "Organization"}
            </p>
          </div>
        </div>
      ...
      <div className="card">
        <div className="row wrap">
          <h3 className="grow" style={{ margin: 0 }}>Network intelligence</h3>
