...
import { api, type GraphEdge, type GraphResult, type Insight, type SearchHit } from "../api";
...
export default function Explore() {
  ...
  // Client-side node-type filter (server expands full subgraph, view slices it).
  ...
  // Fresh expand replaces the view; node-click expand accumulates (1-hop → 2-hop → …).
    ...
    <div>
          ...
          <ul className="list">
            {hits.map((h) => (
              <li key={h.id}>
                ...
                <span className="grow"><strong>{h.name}</strong></span>
                <button className="btn-sm" onClick={() => doExpand(h.id, false)}>Expand</button>
              </li>
            ))}
          </ul>
        )}
      ...
      <div className="explore-grid mt">
        <FilterPanel
          ...
          nodeTypes={nodeTypes} setNodeTypes={setNodeTypes}
        />
            ...
            <div className="card">
              ...
              <ul className="list">
                {insight.facts.map((f, i) => (
                  <li key={i} className="mono">{f.text}</li>
                ))}
              </ul>
